// tool-schema.ts - Tool schema extraction and processing
...
function extractInputSchema(tool: any): any {
  ...
  // Handle different wrapping formats
...
/**
 * Creates a schema object for Bedrock API
 *
...
export function processToolsForClaude(tools: any[]): ProcessedTools {
  ...
  const processedTools = tools.map((tool: any) => {
    ...
    // Store mapping from base name to full name
    toolMapping[baseName] = tool.name

    // Truncate main description if necessary
